...
문서 버전: 1.6  
작성일: 2026-02-25  
코드 기준: `App/Program.cs`, `UI/MainWindow.cs`, `Networking/ClientHandler.cs`, `Core/Protocol/ProtocolPayloadLimits.cs`, `SharpKVM.csproj`, `SharpKVM.sln`
...
- `ClipboardImage(9)`
- `PlatformInfo(10)`
- `ClientDiagnosticLog(11)`: 클라이언트 진단 로그 payload(UTF-8 텍스트) 서버 업로드
...
- `Clipboard*` payload는 타입별 상한 검증 후 처리 (초과/비정상 길이는 연결 종료 처리)
...
- macOS 클라이언트 `Client` 탭에서 `Enable CapsLock Input Source Switch` 체크박스로 CapsLock 입력소스 전환 허용/차단을 수동 제어
- 체크박스는 macOS에서만 표시되며, 연결 시 초기값은 감지된 옵션값으로 자동 동기화
- `CapsLock` 단독 입력은 symbolic hotkey 감지와 별도로 전용 토글 경로를 통해 입력소스 전환 처리
- 서버는 원격 클라이언트 전환/로컬 복귀/서버 중지 시 추적 중인 키를 강제 `KeyUp` flush하여 원격 modifier 고착 상태를 방지
- macOS 입력 진단 키 로그에는 `CapsLock` 외 `Hangul` 키도 포함
- macOS 클라이언트는 `CapsLock`/입력소스 hotkey 실행 시 전후 입력소스 상태를 캡처해 `switched=true/false/unknown` 진단 로그를 남김
- 해당 진단 로그는 `ClientDiagnosticLog(11)` payload로 서버에 전달되어 서버 로그에도 `[ClientDiag][Client-...]` 형태로 기록
- `CapsLock` 직접 토글 경로에서 `switched!=true`가 감지되면, 가능한 경우 symbolic input source hotkey(60/61) 실행으로 1회 이상 자동 폴백 시도
- symbolic hotkey를 로드하지 못한 경우(`skipped_no_hotkeys`)에도 기본 `Ctrl+Space` 조합으로 추가 폴백 시도
- 기본 `Ctrl+Space` 폴백은 `osascript` + `System Events key code 49 using control down` 경로를 사용
- macOS 입력소스 plist 진단이 실패하면 체크박스를 강제로 `OFF`로 덮어쓰지 않고 현재 사용자 토글 값을 유지
- macOS 클라이언트는 접근성 권한 상태(`AXIsProcessTrusted`)를 주기적으로 로그에 출력
...
- 압축 해제 시 zip entry 경로 정규화/루트 경계 검증으로 path traversal 차단
- 압축 해제 총량 상한(500MB) 적용
...
- payload 상한: 텍스트 1MB, 파일(zip) 100MB, 이미지 50MB
...
- macOS 입력소스 symbolic hotkey의 `CapsLock` 자기 비트(`0x00010000`)는 단독 전환 판정 시 정규화되어 처리됨 (`DEFECT-MAC-CAPSLOCK-001` 대응)
